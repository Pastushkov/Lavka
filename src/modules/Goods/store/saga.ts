...
import { getSoapList, getSoapById, updateTranslation, updateProduct } from "./api/goods.api";
...
function* fetchUpdateProductSaga({ payload }: any) {
  try {
    console.log(payload);
    const response: AxiosResponse<{ payload: any }> = yield call(updateProduct, payload);
    console.log(response);
  } catch (error: any) {
    yield put({
      type: shopTypes.ERROR_UPDATE_TRANSLATION,
...
function* shopSaga(): Generator<AllEffect<ForkEffect<never>>, void, unknown> {
  yield all([
    ...
    takeLatest(shopTypes.FETCH_UPDATE_PRODUCT,fetchUpdateProductSaga)
  ]);
}
